...
public class InMemoryCabLockProvider implements CabLockProvider {
  ...
  @Override
  public synchronized void lockCabs(List<Cab> cabs, String userId) throws CabTemporarilyUnavailable {
    for (Cab cab : cabs) {
      if (Boolean.TRUE.equals(isCabLocked(cab))) {
  ...
  public void unlockCabs(List<Cab> cabs, String userId) {
    for (Cab cab : cabs) {
      if (Boolean.TRUE.equals(validateLock(cab, userId))) {
        unlockCab(cab);
      }
    }
  }
